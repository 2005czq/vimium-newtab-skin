...
.znt-clock {
  ...
  line-height: 1;
  letter-spacing: 0;
  font-feature-settings: "tnum" 1, "cv01", "cv02", "cv61", "cv66", "cv97", "ss03", "ss05", "ss07", "ss08", "ss11";
  font-variant-numeric: tabular-nums;
  text-shadow:
    0 0 18px rgb(var(--znt-lavender) / 0.32),
    0 0 44px rgb(var(--znt-mauve) / 0.16);
}
...
.znt-date {
  ...
  letter-spacing: 0;
  font-variant-numeric: tabular-nums;
  text-shadow: 0 0 16px rgb(var(--znt-lavender) / 0.18);
}
...
.znt-footer {
  ...
  z-index: 4;
  width: min(600px, calc(100vw - 112px));
  display: flex;
  flex-direction: column;
  align-items: center;
  color: rgb(var(--znt-mauve));
